    ...
    return (
        <>
            <div className="flex flex-col p-6 pb-1 bg-white rounded-lg shadow-md w-[calc(50%-1rem)] relative">
                <div className={`h-12 text-white text-center rounded-lg mb-2 font-bold flex flex-col justify-center items-center ${shortcut.removable ? 'bg-teal-500' : 'bg-green-500'}`}>
                    {!showDeleteConfirm ?
                        <>
                            {/* 快捷键 */}
                ...
                <div className="flex justify-center items-center">
                    ...
                    <div className="flex items-center p-2">
                        {shortcut.gestureLeft ? (
                            <img src={`./images/hands/${shortcut.gestureLeft}_Left.png`} className="w-32 h-32 object-cover mr-2" />
                        ) : (
                            <div className="w-24 h-24 border-dashed border-2 border-teal-600 mr-8" />
                        )}

                        {shortcut.gestureRight ? (
                            <img src={`./images/hands/${shortcut.gestureRight}_Right.png`} className="w-32 h-32 object-cover ml-1" />
                        ) : (
